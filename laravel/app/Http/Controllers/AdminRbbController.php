...
class AdminRbbController extends Controller
{
    function index() {
        $db_target = DB::connection('db_target');
        $kat_vol = $db_target->select("
            SELECT a.id,a.id_unit,b.name from new_master_category as a join new_master_link as b on a.id_link=b.id where a.id_unit=3
        ");
        // return $kat_vol;
        ...
        return view('admin.target.rtb',$data);
    }
    function new(Request $req) {
        ...
        join new_master_category as c on t.id_category=c.id 
        join new_master_link as l on c.id_link=l.id
        where c.id_unit = 3 and t.thbl=".$thbl." and l.id='".$req->kategori."'");
        $count = count($cek);
        // return $count;
        if($count == 1){
    ...
    function edit($id) {
        $db_target = DB::connection('db_target');
        $kat_vol = $db_target->select("
            select a.id,a.id_unit,a.id_link,b.name from new_master_category as a join new_master_link as b on a.id_link=b.id where a.id_unit=3
        ");
        // return $kat_vol;
        ...
        return view('admin.target.edit_volume.edit_rtb',$data);
    }
    function post_edit(Request $req,$id) {
        ...
        $new_target->save();
        return redirect('/admin/ratuboko')->with('sukses','Berhasil');
    }
}
