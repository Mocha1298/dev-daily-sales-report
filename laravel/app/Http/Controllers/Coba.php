            ...
            (
                ...
                (
                    (
                        ...
                        WHERE a.group_id = 4 AND a.trx_date = (CURRENT_DATE - 1) and a.ctg_id in (4,5,6) AND (b.trf_trfftype_id NOT IN (2) and trf_name not like '%Meals%') 
                    ...
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND a.trx_date = (CURRENT_DATE - 1) and a.ctg_id not in (4,5,6) AND (b.trf_trfftype_id NOT IN (2) and trf_name not like '%Meals%') 
                        group by a.trx_date, id_link
                    )
                    union all
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND a.trx_date = (CURRENT_DATE - 1) AND (b.trf_trfftype_id IN (2) or trf_name like '%Meals%') 
                        group by a.trx_date, id_link
                    )
                )x
            ...
            (
                ...
                (
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        where a.group_id = 4 and TO_CHAR(a.trx_date::date, 'yyyymm') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyymm') and a.ctg_id in (4,5,6) and b.trf_trfftype_id NOT IN (2) and trf_name not like '%Meals%'
                    ...
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        where a.group_id = 4 and TO_CHAR(a.trx_date::date, 'yyyymm') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyymm') and a.ctg_id not in (4,5,6) and b.trf_trfftype_id NOT IN (2) and trf_name not like '%Meals%'
                        group by thbl,  id_link
                    )
                    union all
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        where a.group_id = 4 and TO_CHAR(a.trx_date::date, 'yyyymm') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyymm') AND (b.trf_trfftype_id IN (2) or trf_name like '%Meals%')
                        group by thbl,  id_link
                    )
                )y
            ...
            (
                ...
                (
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        where a.group_id = 4 and TO_CHAR(a.trx_date::date, 'yyyy') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyy') and a.ctg_id in (4,5,6) and b.trf_trfftype_id NOT IN (2) and trf_name not like '%Meals%'
                    ...
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        where a.group_id = 4 and TO_CHAR(a.trx_date::date, 'yyyy') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyy') and a.ctg_id not in (4,5,6) and b.trf_trfftype_id NOT IN (2) and trf_name not like '%Meals%'
                        group by tahun,  id_link
                    )
                    union all
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        where a.group_id = 4 and TO_CHAR(a.trx_date::date, 'yyyy') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyy') AND (b.trf_trfftype_id IN (2) or trf_name like '%Meals%')
                        group by tahun,  id_link
                    )
                )z
            ...
            (
                ...
                (
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND a.trx_date = (CURRENT_DATE - 1) AND b.trf_trfftype_id NOT IN (2) and b.trf_name not like '%Meals%' and a.source_type not in ('B2B','OTA') 
                        group by a.trx_date, id_link
                    )
                    union all
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND a.trx_date = (CURRENT_DATE - 1)  AND (b.trf_trfftype_id IN (2) or b.trf_name like '%Meals%') and a.source_type not in ('B2B','OTA')
                        group by a.trx_date, id_link
                    )
                )x
            )x 
            ...
            (
                ...
                (
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND TO_CHAR(a.trx_date::date, 'yyyymm') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyymm') AND b.trf_trfftype_id NOT IN (2) and b.trf_name not like '%Meals%' and a.source_type not in ('B2B','OTA') 
                        group by thbl, id_link
                    )
                    union all
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND TO_CHAR(a.trx_date::date, 'yyyymm') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyymm') AND (b.trf_trfftype_id IN (2) or b.trf_name like '%Meals%') and a.source_type not in ('B2B','OTA')
                        group by thbl, id_link
                    )
                )y
            )y on w.id_link = y.id_link
            ...
            (
                ...
                (
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND TO_CHAR(a.trx_date::date, 'yyyy') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyy') AND b.trf_trfftype_id NOT IN (2) and b.trf_name not like '%Meals%' and a.source_type not in ('B2B','OTA') 
                        group by tahun, id_link
                    )
                    union all
                    (
                        ...
                        master_tariff b 
                        on a.trf_id = b.trf_id
                        WHERE a.group_id = 4 AND TO_CHAR(a.trx_date::date, 'yyyy') = TO_CHAR((CURRENT_DATE - 1)::date, 'yyyy')  AND (b.trf_trfftype_id IN (2) or b.trf_name like '%Meals%') and a.source_type not in ('B2B','OTA')
                        group by tahun, id_link
                    )
                )z
            )z on w.id_link=z.id_link
...
(
 ...
 on x.id_link = z.id_link
        ...
        return view("prambanan",['ticketing'=>$ticketing,'target_pengguna_jasa'=>$target_pengguna_jasa,'income'=>$income,'target_income'=>$target_income,'erp'=>$erp,'target_income_non'=>$target_income_non,'count'=>$count]);
    }
